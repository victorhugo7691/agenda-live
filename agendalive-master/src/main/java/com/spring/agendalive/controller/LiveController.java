...
import com.spring.agendalive.service.LiveResultService;
...
public class LiveController {
    ...
    @Autowired
    LiveResultService service;

    @GetMapping("/lives")
    public ResponseEntity<List<LiveDocument>> getAllLives(
            @PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.ASC) Pageable pageable,
            @RequestParam(required = false) String flag) {
        List<LiveDocument> livePage = service.findAll();
        if (livePage.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            return new ResponseEntity<List<LiveDocument>>(livePage, HttpStatus.OK);
        }
    }

    @GetMapping("/lives/{id}")
    public ResponseEntity<LiveDocument> getOneLive(@PathVariable(value = "id") String id) {
        LiveDocument liveO = this.service.findByID(id);
        if (liveO == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            return new ResponseEntity<LiveDocument>(liveO, HttpStatus.OK);
        }
    }
